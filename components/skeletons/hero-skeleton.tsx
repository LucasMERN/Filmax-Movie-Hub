import { Carousel, CarouselContent, CarouselItem } from '@/components/ui/hero-carousel';
  ...
  return (
    <>
      <Skeleton
        ...
        className="h-168.75 w-full rounded-none bg-cover bg-center lg:h-212.5"
      />
      <div className="relative container mx-auto h-162.5 pt-36 text-3xl font-bold text-white lg:h-197.5 lg:text-6xl">
        ...
        <div className="-m-4 mt-16 -mb-72 flex flex-col gap-4 rounded-2xl bg-black/60 p-4 sm:w-1/2 lg:hidden">
          <Skeleton className="h-16 w-1/4" />
          <span className="inline-flex h-8 w-fit items-center justify-center rounded-md bg-white px-4 text-base font-semibold whitespace-nowrap text-secondary-foreground transition-colors hover:bg-secondary/80">
            Explore
          </span>
        </div>
        <Carousel
              ...
              return (
                <CarouselItem href="#" key={index} className="basis-1/3">
                  ...
                  >
                    ...
                    {index == 8 ? <Skeleton className="mt-2 h-6 w-full text-center text-lg" /> : ''}
                  </div>
                </CarouselItem>
              );
            })}
